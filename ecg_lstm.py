...
parser.add_argument('--start-epoch', default=44, type=int, metavar='N',
...
pp = '/home/ishaan/work_dir/ecg_spectogram/ecg_phase6/checkpoints/phy_time_spectro_tencoder/checkpoint.pth.tar'

parser.add_argument('--resume', default=pp, type=str, metavar='PATH',
...
def evaluate(outputs, labels, label_names=None):
    ...
    report = skm.classification_report(
        gt, pred,
        target_names=[name_dict[i] for i in np.unique(gt)],#label_names[:min(np.unique(gt).shape[0], 6)],
        digits=3)
...
def main():
    ...
    model = models.__dict__['resnet_lstm'](
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(trainloader)):
        ...
        # outputs = torch.reshape(outputs, (outputs.size(0)*outputs.size(1), 6))
        # targets = torch.reshape(targets, (targets.size(0)*targets.size(1),))
