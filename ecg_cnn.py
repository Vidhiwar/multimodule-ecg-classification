...
parser.add_argument('-ft', '--transformation', default='cwt', type=str)
...
parser.add_argument('--block-name', type=str, default='BasicBlock',
                    help='the building block for Resnet and Preresnet: BasicBlock, Bottleneck (default: '
                         'Basicblock for ecg classification)')
...
class Ecg_loader(torch.utils.data.Dataset):
    def __init__(self, path, transform):
        ...
        for i in tqdm(data):
            ...
            img = cv2.imread(os.path.join(path, 'images_full', transform, i['images_full']))
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            l = i['label']
            a = i['age']
            g = i['gender']
            self.inputs.append(img.transpose((2, 0, 1)))
...
def main():
    ...
    model = models.__dict__['resnet_cnn'](
    ...
    # Resume
    title = 'ecg-cnn-resnet' + str(args.depth)
    if args.resume:
        # Load checkpoint.
